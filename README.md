...
| CSE | thecse.com's bulletins sitemap — the bulletin type (name change / symbol change / name & symbol change) is encoded in the URL slug itself. Old name/ticker is enriched in from thecse.com's listed-companies webapi, which carries a `recent_change` field (`name_was`/`symbol_was`) for each company's single most recent change — matched to a bulletin only when the bulletin's own date exactly equals that change's effective date, so it only fills in when the bulletin *is* the company's latest change (CSE companies often rename again within days, so a loose date match risks attaching the wrong old name) |
