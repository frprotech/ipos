...
1. The **Update IPO data** workflow (`.github/workflows/update-ipos.yml`) runs
   every 6 hours (and on demand from the Actions tab). RTO data has its own
   separate **Update RTO data** workflow (`.github/workflows/update-rtos.yml`,
   also every 6 hours) so a slow RTO fetch (the NASDAQ/NYSE source downloads a
   large SEC data file) never delays IPO updates, and vice versa.
...
reverse takeovers — updated on its own 6-hour cycle (a separate workflow from
the IPO one, see below) by `scripts/fetch_rtos.py` into `data/rtos.json`:
...
| CSE | thecse.com's bulletins sitemap — the bulletin type (name change / symbol change / name & symbol change) is encoded in the URL slug itself. Confirmed there's no free source for the former name/ticker directly (bulletin pages are JS-rendered stubs with no former-name text, and no per-company history endpoint exists), so old name/ticker is enriched from thecse.com's listed-companies webapi, which carries a `recent_change` field (`name_was`/`symbol_was`) for each company's single most recent change *at the moment it's checked*. Since that's only one hop and CSE companies often rename again within days, every run appends any newly-seen (ticker, effective date) pairs to `data/snapshots/cse_recent_changes.json` — a bulletin is enriched when ANY accumulated hop for that ticker exactly matches the bulletin's own date, so coverage builds up over time from repeated 6-hourly checks instead of being capped at whatever's live right now |
| NASDAQ / NYSE | SEC EDGAR's bulk `submissions.zip` (one filing-history JSON per company, covering every US-listed CIK) — scanned directly for `formerNames` entries within the tracking window, rather than relying on full-text search for filing wording (which turned out to miss real cases, since not every rename 8-K phrases it the same way). The CIK is a stable ID that never changes, so old and new ticker can be linked even when the ticker itself changes (not just the company name) — this properly covers cases like a reverse takeover that renames both the company and its ticker. Two things this deliberately guards against: SEC's `formerNames` always lists the company's *current* name too with a "to" date that rolls forward to today (that's bookkeeping for "still valid as of now", not a real change, so it's skipped), and a CIK's `tickers` list includes every security it has ever filed under — preferred share series, OTC/foreign cross-listings, sponsored ETNs — not a rename history, so only a same-CIK ticker that looks like plain common stock on a tracked exchange is ever treated as "the old ticker". |
...
1. **Enable the workflows** — merge this branch to `main`; scheduled workflows
   only run from the default branch.
2. **Enable GitHub Pages** — repo *Settings → Pages → Source: GitHub Actions*.
   Either workflow then publishes the site automatically after its data update.
3. **First data load** — go to *Actions → Update IPO data → Run workflow*, and
   separately *Actions → Update RTO data → Run workflow*, to populate both
   tables immediately instead of waiting for the next cron tick.
...
command, no framework. Because both update workflows commit their refreshed
data file (`data/ipos.json` / `data/rtos.json`) to the branch, every data
update automatically triggers a new Vercel deployment, so the hosted tables
stay current. Each workflow's `deploy` job (GitHub Pages) is optional and can
be deleted from `.github/workflows/update-ipos.yml` /
`.github/workflows/update-rtos.yml` if you only use Vercel.
